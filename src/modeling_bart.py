...
class BartNarrationModel(nn.Module):
    def __init__(self, vocab_size, model_type,
     modelbase='facebook/bart-base', share_mpu_sa=False,device=torch.device(
    'cuda') if torch.cuda.is_available() else torch.device('cpu')):
        ...
        self.model_type = self.bartconfig.modeltype = model_type
        ...
        self.bartconfig.share_mpu_sa=share_mpu_sa
        self.device = device
    ...
    def FusionModelsTraining(self,batch):
        device = self.device
        met, rate, val = batch['metrics_seq'].to(device), batch['rates'].to(device), batch['values'].to(device)
    ...
    def baselineTraining(self, batch):
        preamble_tokens = batch['preamble_tokens']
        preamble_attention_mask = batch['preamble_attention_mask']
        labels = batch['labels']
